...
<div id="action" class:hidden="{$pageCount === 1}">
  <button class="left" disabled="{$currentPage === 1}" on:click="{() => $currentPage--}"
    ><Icon {...leftProps} /></button
  >
  <button class="right" disabled="{$currentPage >= $pageCount}" on:click="{() => $currentPage++}"
    ><Icon {...rightProps} /></button
  >
</div>
