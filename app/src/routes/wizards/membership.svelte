<script>
    ...
    $: {
        provider?.isMetaMaskInstalled()
            .then(result => isMetamaskInstalled = result);
    }
    ...
    async function buyTokens() {
        let response = await provider.Plugins.fiat.buy({}, { disableTriggers: true, defaultAmount: 10 });
    ...
    function isValidContact() {
        return firstName && firstName.length > 0 && lastName && lastName.length > 0 && email && email.length > 0
    }
    ...
    async function handleAddAvalanche() {
        ...
        await provider.addNetwork( 43113, "Avalanche Testnet", "AVAX", "AVAX",
            "https://api.avax-test.network/ext/bc/C/rpc", "https://testnet.snowtrace.io/"
        );
    }
    ...
    function shortEthAddress(ethAddress) {
        if (ethAddress){return ethAddress.slice(0,5) + "..." + ethAddress.slice(-4)}
        else{return null;}
    }    
</script>
...
<section id="content">
    {#if $currentPage === 1 && isMember === true}
        ...
        <p>
            You're already a member, thank you very much!
    ...
    {:else if $currentPage === 1}
        ...
        <p>
            To take advantage of them, a bit of preparation is needed.
        </p>
    ...
    {:else if ($currentPage === 3)}
        ...
        <p>
            Your crypto wallet securely stores your digital goods.
            Connect to one of our wallet providers or create a new one.
        </p>
        <p id="buttons">
            First, please connect your wallet with us.
        </p>
        ...
        <p id="buttons">
            Then, let's switch to the Avalanche chain.
        </p>
        <button on:click={handleAddAvalanche}>
            Add Avalanche to wallet
        </button>

    {:else if ($currentPage === 4)}
        <h2>Fund your wallet</h2>
        <p>
            You are going to need some AVAX tokens to mint your NFT.
            Don't worry it's not real money just test crypto.
        </p>
        <p>Click on Copy Wallet Address, followed by Request AVAX. When complete, come back to continue.</p>
        <div id = "fund_wallet">
            ...
            <a id="avax-faucet" href="https://faucet.avax-test.network/" target="_blank" rel="noopener noreferrer">
    ...
    {:else if ($currentPage === 5)}
        <h2>ourworlds<sup>+</sup></h2>
        <p>
            Now we are going to mint your ourworlds NFT!
            Please review that your details are correct and hit apply.
        </p>
        <dl id="review">
            <dt>Contact</dt>
        ...
        <div id="action">
            <button on:click={handleSubmit} disabled="{!isValidContact() || !isValidWallet() || isMember}">
