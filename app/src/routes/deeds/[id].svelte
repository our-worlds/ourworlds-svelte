<script>
    ...
    async function checkUserState() {
        ...
        return Promise.all([creator, participant, participating])
            .then(async values => {
                ...
                if (participating?.id === deedId)
                    userState = USERSTATE.participant;
                else if (participating)
                    userState = USERSTATE.participating;    // Participating in another deed already
                else if (values[1].get('isMember'))
                    userState = USERSTATE.member;           // Only members can participate
                else
...
<section id="grid">
    ...
    <div id="action">
        ...
        {:else if userState === USERSTATE.participant}
            ...
            <button style="background-color: #79C679"
                on:click={handleParticipantRequest}
            >
            Complete Deed</button>
        {:else if (userState === USERSTATE.member)}
            <!-- user can participate in one deed at a time only-->
            <button disabled="{userState === USERSTATE.participating}" on:click={handleParticipate}>Participate</button>
        {:else}
    ...
    {#if userState === USERSTATE.creator}
        <div id="participants-container">
            ...
            <p>
                When the user request to compelte the deed, please confirm
                if they have completed the required steps.
            </p>
                ...
                {#if participants.length > 0}
                    {#each participants as participant}
                        <li class="card" data-id="{deed.id}">
                            ...
                            <button disabled={!participant.isRequestingCompletion} on:click={() => {handleDeedCompletion(participant)}}>
                                <span>✔️</span>
                            </button>
                        </li>
                    {/each}
