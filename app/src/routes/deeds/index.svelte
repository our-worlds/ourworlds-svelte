...
<style>
    ...
    #deeds-container #activecover, #deeds-container #inactivecover {
        height: 141px;
        border-radius: 16px;
    }
...
<NavBar fixed={true}>
    <h1>Explore</h1>
    {#if loggedIn}
        <button class="log-button" on:click={handleLogout}>
            Log out
        </button>
    {:else}
        <button class="log-button" on:click={handleLogin}>
            Log in
        </button>
    {/if}
</NavBar>
...
<section>
        ...
        {#if activeDeed || deedsCreated.length !== 0}
            <div id="deeds-container">
                ...
                {#if (deedsCreated?.length > 0)}
                    <h2>Created Deeds</h2>
                    <Slider bind:deedList="{deedsCreated}" on:deedClick={event => goto(`/deeds/${event.detail.id}`)} />
                {/if}
            </div>
        {:else}
    ...
    <div id="action">
        <button id="search" on:click={() => goto('/deeds/map')}><Icon {...searchProps} /><span>Discover Deeds</span></button>
