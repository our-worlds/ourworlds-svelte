...
<style>
    ...
    .input input, .location button, .upload label, .area textarea, .list dt button, #action {
        height: 48px;
    ...
    .upload label, .list dt button, .list dd {
        display: flex;
        align-items: center;
        justify-content: space-between;
    ...
    .input label, .location label, .area label {
        width: fit-content;
...
<section>
    ...
    <div class="location">
        ...
        <button id="location" on:click={() => goto('/deeds/selectlocation')}>
            <Icon {...markerProps} />
            <span>{$tempDeed.address ? `${$tempDeed.coordinates.latitude}, ${$tempDeed.coordinates.longitude}` : 'Tap to set location'}</span>
        </button>
    </div>
    <div class="upload">
        <label for="picture">
