<style>
  :root {
    ...
    --brand-dark: hsl(
      var(--brand-hue) calc(var(--brand-saturation) / 2) calc(var(--brand-lightness) / 1.5)
    );
...
<script>
  ...
  const getPseudoStyle = (element, prop) =>
    parseInt(window.getComputedStyle(element, ':before').getPropertyValue(prop));
  ...
  function determineChecked() {
    let curpos = Math.abs(
      Number.parseInt(switchElement.style.getPropertyValue('--thumb-position')),
    );
