...
<script>
    ...
    function markerClickHandler(event) {
        Array.from(document.querySelectorAll('.activeMarker')).forEach((el) => el.classList.remove('activeMarker'));
        event.target._icon.classList.add('activeMarker');
        const nodes = document.querySelectorAll(`[data-id="${event.target.getElement().firstElementChild.dataset.id}"]`);
        nodes.forEach((item) => {
