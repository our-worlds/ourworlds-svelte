...
<style>
    ...
    #category, #startdate, #reward, #address dt {
        display: flex;
        gap: 20px;
    ...
    #category span:first-child, #startdate span:first-child, #reward span:first-child, #address dt span:first-child, #address dd span:first-child {
        font-weight: 700;
    }
...
<p id="startdate">
    ...
    <span>{deed?.timeframe?.start ? new Date(deed?.timeframe.start).toLocaleString() : 'None'}</span></p>
